...
- `SLIDES`: array of slides in display order. Fields:
  ...
  - `media`: `null`, or `{ type: "image", src: "/media/x.jpg" }`, or `{ type: "video", src: "/media/x.mp4" }`. Videos autoplay muted and looped, only on the current slide. Optional `position` (CSS `object-position`, e.g. `"center 15%"`) picks which part of the image or video stays in frame when it's cropped. Images can instead use `fit: "contain"` to show the whole photo, uncropped, over a dimmed blurred copy of itself.
